...
> **Назначение этого файла.** Единый источник правды о **пошаговой Python-игре** (прод).
> Для автосима (веб-порт UE5-песочницы) — см. [SANDBOX_CONTEXT.md](SANDBOX_CONTEXT.md).

## Параллельные треки

Проект существует в двух независимых ветках разработки:

| Трек | Файл контекста | Стек | Статус |
|---|---|---|---|
| **Evo4X** — пошаговая игра с игроком | этот файл | Python + Streamlit | Прод, активно |
| **Sandbox** — пространственный автосим | [SANDBOX_CONTEXT.md](SANDBOX_CONTEXT.md) | TypeScript + Vite + SVG (порт с UE5) | Прототип |

Движки **не совместимы** по логике: разные механики миграции, масштаб карты, нет общего игрока.
Код не переиспользуется между треками.

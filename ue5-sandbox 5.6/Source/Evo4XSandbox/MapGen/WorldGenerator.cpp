...
void FWorldGenerator::Generate(const FSimConfig& Config, FHexGrid& OutGrid, FWorldState& OutWorld)
{
	...
	// Step 3b: Fix disconnected fragments (enclaves, thin tendrils)
	FixDisconnectedRegions(OutGrid);
...
void FWorldGenerator::GenerateRegions(FHexGrid& Grid, int32 NumRegions, int32 MinSize, const FString& Seed)
{
	...
	while (bChanged)
	{
		...
		for (int32 RId = 0; RId < ActualRegions; ++RId)
		{
			...
			// Pick the most compact candidate: prefer tiles adjacent to more same-region tiles
			int32 BestScore = -1;
			TArray<int32> BestCandidates;
			for (const int32 CandIdx : Candidates)
			{
				int32 Score = 0;
				for (const int32 Nb : Grid.GetTileNeighbors(CandIdx))
				{
					if (Grid.Tiles[Nb].RegionId == RId) Score++;
				}
				if (Score > BestScore)
				{
					BestScore = Score;
					BestCandidates.Reset();
					BestCandidates.Add(CandIdx);
				}
			...
			const int32 PickIdx = FEvoRng::ChooseIndex(Seed, PickKey, BestCandidates.Num());
			const int32 ClaimedTile = BestCandidates[PickIdx];
...
void FWorldGenerator::FixDisconnectedRegions(FHexGrid& Grid)
{
	// Run repeatedly until stable (small regions may become disconnected after a merge)
	bool bAnyFixed = true;
	while (bAnyFixed)
	{
		bAnyFixed = false;

		// Build per-region tile list
		TMap<int32, TArray<int32>> RegionTiles;
		for (const int32 LandIdx : Grid.LandIndices)
		{
			RegionTiles.FindOrAdd(Grid.Tiles[LandIdx].RegionId).Add(LandIdx);
		}

		for (auto& Pair : RegionTiles)
		{
			const int32 RegionId = Pair.Key;
			const TArray<int32>& TileList = Pair.Value;
			if (TileList.Num() <= 1) continue;

			// BFS flood fill from first tile to find connected component
			TSet<int32> Visited;
			TArray<int32> Queue;
			Queue.Add(TileList[0]);
			Visited.Add(TileList[0]);

			for (int32 qi = 0; qi < Queue.Num(); ++qi)
			{
				for (const int32 Nb : Grid.GetTileNeighbors(Queue[qi]))
				{
					if (Grid.Tiles[Nb].RegionId == RegionId && !Visited.Contains(Nb))
					{
						Visited.Add(Nb);
						Queue.Add(Nb);
					}
			...
			// Reassign any tiles not reachable from the main component
			for (const int32 TileIdx : TileList)
			{
				if (Visited.Contains(TileIdx)) continue;

				// Find a neighboring region to absorb this tile
				int32 BestRegion = -1;
				for (const int32 Nb : Grid.GetTileNeighbors(TileIdx))
				{
					if (Grid.Tiles[Nb].bIsLand && Grid.Tiles[Nb].RegionId != RegionId)
					{
						BestRegion = Grid.Tiles[Nb].RegionId;
						break;
					}
				}
				...
				if (BestRegion >= 0)
				{
					Grid.Tiles[TileIdx].RegionId = BestRegion;
					bAnyFixed = true;
				}
			}
		}

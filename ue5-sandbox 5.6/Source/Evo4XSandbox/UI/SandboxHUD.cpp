...
FString ASandboxHUD::SpeedString() const
{
	...
	case ESimSpeed::VeryFast: return TEXT("TURBO (2t/s)");
	default:                  return TEXT("--");
	}
}
...
void ASandboxHUD::DrawHUD()
{
	...
	DrawControlsHint(20.f, Canvas->ClipY - 170.f);

	// Region inspector: right side
	if (SelectedRegionId >= 0 && CachedWorld->Regions.IsValidIndex(SelectedRegionId))
	{
		DrawRegionInspector(Canvas->ClipX - 540.f, 20.f);
	}

	// History graph: bottom-left
	DrawHistoryGraph(20.f, Canvas->ClipY - 440.f, 500.f, 240.f);
}
...
void ASandboxHUD::DrawGlobalPanel(float X, float Y)
{
	...
	if (!CachedWorld->Metrics.FastestGrowing.IsNone())
	{
		...
		DrawTextShadow(FString::Printf(TEXT("Growing: %s"), *Gr.UiName),
			X, CurY, FLinearColor(0.3f, 1.f, 0.3f), 2.0f);
	}
	CurY += LineH;
	...
	if (!CachedWorld->Metrics.FastestDeclining.IsNone())
	{
		...
		DrawTextShadow(FString::Printf(TEXT("Declining: %s"), *Dc.UiName),
			X, CurY, FLinearColor(1.f, 0.3f, 0.3f), 2.0f);
	}
}
...
void ASandboxHUD::DrawRegionInspector(float X, float Y)
{
	...
	{
		DrawTextShadow(TEXT("Active Effects:"), X, CurY, FLinearColor(1.f, 0.6f, 0.2f), 1.8f);
		CurY += LineH;
		for (const FActiveEffect& Eff : Region.ActiveEffects)
		{
			DrawTextShadow(FString::Printf(TEXT("  %s (TTL %d) R1:%.0f%% R2:%.0f%% R3:%.0f%%"),
				*Eff.EffectType.ToString(), Eff.TTL,
				...
				X, CurY, FLinearColor(1.f, 0.8f, 0.4f), 1.6f);
			CurY += LineH;
		}
	}
	else
	{
		DrawTextShadow(TEXT("No active effects"), X, CurY, FLinearColor(0.5f, 0.5f, 0.5f), 1.6f);
		CurY += LineH;
	}
	...
	// Species list
	DrawTextShadow(TEXT("Species:"), X, CurY, FLinearColor::White, 2.0f);
	CurY += LineH;

	// Header row
	DrawTextShadow(TEXT("Name        Pop    Share  Trend  Out   In"),
		X, CurY, FLinearColor(0.7f, 0.7f, 0.7f), 1.6f);
	CurY += LineH;
	...
	for (const auto& Pair : Region.Populations)
	{
		...
		DrawTextShadow(Line, X, CurY, Spec.Color, 1.7f);
		CurY += LineH;
	}
	...
	// Neighbors
	DrawTextShadow(TEXT("Neighbors:"), X, CurY, FLinearColor::White, 1.8f);
	CurY += LineH;
	...
	DrawTextShadow(NeighborStr, X, CurY, FLinearColor(0.6f, 0.8f, 1.f), 1.5f);
}
...
{
	DrawRect(FLinearColor(0.f, 0.f, 0.f, 0.5f), X - 5.f, Y - 5.f, 550.f, 160.f);

	const float LineH = 32.f;
	DrawTextShadow(TEXT("[Space] Pause/Resume  [1] Slow  [2] Fast  [3] Turbo"),
		X, Y, FLinearColor(0.7f, 0.7f, 0.7f), 1.7f);
	DrawTextShadow(TEXT("[T] Step one tick  [O] Cycle overlay"),
		X, Y + LineH, FLinearColor(0.7f, 0.7f, 0.7f), 1.7f);
	DrawTextShadow(TEXT("[WASD] Pan  [Scroll] Zoom  [Click] Select"),
		X, Y + LineH * 2, FLinearColor(0.7f, 0.7f, 0.7f), 1.7f);
	DrawTextShadow(TEXT("[Esc] Deselect region"),
		X, Y + LineH * 3, FLinearColor(0.7f, 0.7f, 0.7f), 1.7f);
}
...
void ASandboxHUD::DrawHistoryGraph(float X, float Y, float W, float H)
{
	...
	DrawTextShadow(TEXT("Population History"), X, Y - 30.f,
		FLinearColor::White, 1.7f);
	...
	for (const auto& Pair : CachedWorld->GlobalPopHistory)
	{
		...
		if (Hist.Num() > 0)
		{
			...
			DrawTextShadow(Spec.DisplayChar, LabelX, LabelY, Color, 1.4f);
		}
	}
}
